...
class LineControlNode(Node):
    def __init__(self):
        ...
        self.steering_kp = 1.0           # proportional gain on normalized near-band offset
        self.lookahead_steering_kp = 1.5 # gain when only the far band sees the line (corner entry)
        self.base_speed = 0.25           # forward speed on a straight (m/s)
        self.min_speed = 0.16            # forward speed in tightest corner / lookahead-only mode
