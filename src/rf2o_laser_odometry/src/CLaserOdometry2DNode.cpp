...
CLaserOdometry2DNode::CLaserOdometry2DNode(): Node("CLaserOdometry2DNode")
{
  ...
  // NOTE: this rover's rplidar_node publishes /scan as RELIABLE (not the
  // sensor_data default of BEST_EFFORT). A best_effort sub *should* accept a
  // reliable pub per the spec, but rmw_fastrtps_cpp on Humble has been
  // observed to silently drop traffic in that combination. Match the
  // publisher exactly.
  laser_sub = this->create_subscription<sensor_msgs::msg::LaserScan>(laser_scan_topic,rclcpp::QoS(rclcpp::KeepLast(5)).reliable().durability_volatile(),
